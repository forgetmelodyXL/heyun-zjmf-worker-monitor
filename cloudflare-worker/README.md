...
这是 `server_monitor.py` 的 Cloudflare Worker/D1 版本：支持同时管理多个使用标准魔方财务 API 的 IDC，用 Cron Trigger 定时执行 API / HTTP(S) / TCP 探测，连续失败 3 次后自动重启或开机，并可通过 pushplus、Bark、Telegram、飞书、企业微信、钉钉、Slack、Discord 或自定义 Webhook 通知。
...
魔方财务 API 获取方式：核云用户打开 <https://www.heyunidc.cn/apimanage>，复制登录邮箱或手机号和 API 密钥；其他 IDC 请使用对应网站的账号和密钥。`/apimanage` 是密钥管理页，不是 API 地址；其他 IDC 的 API 地址一般为 `https://你的IDC域名/v1`。产品 ID 可在初始化向导中自动获取，也可以部署后在 `/admin` 手动填写。
...
首次打开 `/admin` 后，在第 2 步添加魔方财务 IDC：

1. 核云选择 **核云（预设）**，系统自动使用 `https://www.heyunidc.cn/v1`。
2. 魔云或其他平台选择 **其他魔方财务 IDC**，填写 IDC 名称、`https://你的IDC域名/v1`、登录账号和 API 密钥。
3. 点击 **自动获取产品列表**，选择服务器并点击 **导入选中服务器**。
4. 如需导入其他 IDC，直接更换 IDC 或账号并重复导入；全部导入后再点击下一步。

系统按“IDC + 产品 ID”保存服务器，不同 IDC 中相同的产品 ID 不会冲突。初始化完成后，可在 **设置 → 服务商设置** 继续新增 IDC；新建监控项时必须选择正确的所属 IDC。
...
`remote_id` 是该 IDC 后台显示的魔方财务产品 ID。系统会自动生成内部服务器 ID；调用服务商 API 时仍只会传原始产品 ID。
